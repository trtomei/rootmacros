...
void MergeRootfile( TDirectory *target, TList *sourcelist, vector<double> crossArray );


void hadd(char* outFileName, char* fileNames) {
  ...
  while( theFile >> theString) {
    theFile >> theWeight;
    theMap.insert(make_pair(theString,theWeight));
  }
  theFile.close();

  // The filenames file.
  theFile.open(fileNames);
  string theFileName;
  // List of Files
  while(theFile >> theFileName) {
    theFile >> tmp;
    cout << theFileName << endl;
    FileList->Add( TFile::Open(theFileName.c_str()) );
    cout << theFileName << " added" << endl;
    double thisWeight = theMap.find(theFileName)->second;
    cout << "weight for this file is " << thisWeight << endl;
    crossSections.push_back(thisWeight);
  }
  // ************************************************************
